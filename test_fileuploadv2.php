...
if(@$_POST['submit'])
{
	...
	if (is_uploaded_file($_FILES['file']['tmp_name']) && $_FILES['file']['error']==0) 
	{
      ...
      if (move_uploaded_file($_FILES['file']['tmp_name'], $path)) {
		...
		$image_up = $_FILES['file']['tmp_name'];
        $imgContent = addslashes(file_get_contents($path));
		//$imgContent = addslashes(file_get_contents($file_up));
	$record= "INSERT INTO img_test2(img)VALUES('$imgContent')";
		...
		if($insert){
			...
			while($row = $result->fetch_assoc()){
				...
				//echo $row['img'];
				//echo '<img src="'.$row['img'].'" height="150px;" width="150px;">';
